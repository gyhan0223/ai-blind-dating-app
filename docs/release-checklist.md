...
- [ ] (#39) 온보딩에 **외모 취향 테스트·외모 중요도·이상형 얼굴 선택 화면이 없다** — 순서: 본인확인 → 얼굴 인증 → 기본 정보 → 자기소개 → 설문 → 가치관 → 선호 조건 → 홈
...
- [ ] (#39) 추천 카드에 자기소개·연애 목적·공개 질문 답변만 보이고, 가치관 설문·민감 응답은 카드/API 응답에 없다
...
- [ ] 마이그레이션 `0015_no_appearance_onboarding.sql` 이 production DB 에 적용되어 있다 (`profiles.intro/relationship_goal/public_answers` 컬럼, `users_guard_onboarding_completion` 트리거)
